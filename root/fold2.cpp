...
public:
  ...
  void myadd(Fraction f1,Fraction f2,Fraction &temp){
    ...
    void mysubs(Fraction f1,Fraction f2,Fraction &temp){
          temp.den=myLcm(f1.den,f2.den);
          temp.num=(temp.den/f1.den*f1.num)-(temp.den/f2.den*f2.num);
          long long divisor;
          divisor=myHcf(temp.den,temp.num);
    ...
    Fraction mymultiply(Fraction f1,Fraction f2,Fraction &temp){
  ...
  for(int i=0;i<t;i++){
    ...
    for(int j=1;j<=n[i];j++){
      if(j%2==0){
        ...
        mysum(start,mymultiply(mysubs(end,start),half),start);
        }
